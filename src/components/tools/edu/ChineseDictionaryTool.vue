<template>
  <div class="chinese-dictionary-tool">
        ...
        <el-tab-pane label="成语接龙" name="solitaire">
          <div class="solitaire-area glass-card">
            ...
            <div class="input-area">
              <el-input
                v-model="userInput"
                placeholder="输入成语进行接龙..."
                size="large"
                clearable
                @keyup.enter="handleIdiomSubmit"
              >
                <template #append>
        ...
        <el-tab-pane label="词义查询" name="lookup">
          <div class="lookup-area glass-card">
            ...
            <div v-if="result" class="search-result animate__animated animate__fadeIn">
              ...
              <h2 class="word-name">
                {{ result.name }} <small v-if="result.pinyin">[{{ result.pinyin }}]</small>
              </h2>
              ...
              <div v-if="result.synonyms || result.antonyms" class="result-grid">
                ...
                <div v-if="result.antonyms" class="grid-item">
                  <h4>反义词</h4>
                  <div class="tag-list">
                    <el-tag v-for="w in result.antonyms" :key="w" type="danger" class="mr-2">{{
                      w
                    }}</el-tag>
                  </div>
                </div>
    ...
    <footer class="footer">© 2026 LRM工具箱 - 成语助手</footer>
  </div>
</template>

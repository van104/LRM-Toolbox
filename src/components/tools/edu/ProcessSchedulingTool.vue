<template>
    <div class="scheduling-tool">
        ...
        <main class="main-content">
            ...
            <section class="card panel-grid">
                <div class="controls-left">
                    <div class="flex-row">
                        ...
                        <div class="control-group">
                            <label>进程数：</label>
                            <input type="number" v-model.number="processCount" min="1" max="26" class="num-input">
                            <button class="btn btn-primary" @click="genDefault(processCount)">生成</button>
                        </div>
                    </div>


                    <div class="algo-options" v-if="selectedAlgo === 'RR' || selectedAlgo === 'MLFQ'">
                        <div class="flex-row">
                            <label>基础时间片 q：</label>
                            <input type="number" v-model.number="timeQuantum" min="1" class="num-input">
                        </div>
                        <div class="info-text" v-if="selectedAlgo === 'MLFQ'">
                            <span>策略：3级队列 (Q0: q, Q1: 2q, Q2: FCFS)</span>
                        </div>
                    </div>
                    <div class="algo-options" v-else-if="selectedAlgo === 'PR'">
                        <div class="flex-row">
                            <label>优先级方向：</label>
                    ...
                    <div class="algo-options info-box" v-else>
                        <span v-if="selectedAlgo === 'SJF'">SJF 为非抢占式（短作业优先），若想要抢占式请选择 SRTF。</span>
                        <span v-else-if="selectedAlgo === 'LJF'">LJF：长作业优先（非抢占），是 SJF 的反面教材。</span>
                        <span v-else-if="selectedAlgo === 'SRTF'">SRTF 为抢占式短剩余时间优先（每时刻可能切换）。</span>
                        <span v-else-if="selectedAlgo === 'LRTF'">LRTF：长剩余时间优先（抢占），是 SRTF 的对立面。</span>
                        <span v-else-if="selectedAlgo === 'HRRN'">HRRN：响应比 = (等待+服务)/服务，非抢占。</span>
                        <span v-else>FCFS：先来先服务（非抢占）。</span>
                    </div>

                    <div class="flex-row speed-control">
                        <label>播放速度: {{ speed.toFixed(1) }}x</label>
                        <input type="range" v-model.number="speed" min="0.2" max="2" step="0.1" class="slider">
                    </div>
                </div>
            ...
            <section class="card" v-if="results.length || timeline.length">
                <div class="gantt-section">
                    <h3>甘特图 (Gantt Chart)</h3>
                    <div class="gantt-wrap">
                        <div class="gantt-chart">
                            <div v-for="(seg, i) in displayedTimeline" :key="i" class="gantt-seg" :style="{
                                width: seg.width,
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr v-for="(p, idx) in processes" :key="idx">
                                ...
                                <td><input type="number" v-model.number="p.arrival" min="0" class="table-input"></td>
                                <td><input type="number" v-model.number="p.service" min="1" class="table-input"></td>
                                <td><input type="number" v-model.number="p.priority" class="table-input"></td>
                                <td>
...
const scheduleSRTF = (raw) => {
    ...
    tl.forEach(s => { finishMap[s.name] = Math.max(finishMap[s.name] || 0, s.end) })
...
const scheduleRR = (raw, q) => {
    ...
    while (idx < procs.length || queue.length) {
        ...
        else { cur.finish = time; finished.push(cur); }
    }
    return finalize(tl, finished);
}
...
const scheduleLRTF = (raw) => {
    ...
    procs.forEach(p => { if (!p.remaining) p.remaining = p.service })
...
const runDispatch = () => {
    ...
    else if (selectedAlgo.value === 'PR') out = schedulePriority(raw, {
        dir: prioDir.value,
...
const palette = ['#0b69ff', '#3d8aff', '#10b981', '#60a5fa', '#f59e0b', '#8b5cf6', '#ef4444', '#ec4899']
const getColor = (name) => {

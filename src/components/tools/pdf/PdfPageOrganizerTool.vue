            ...
            <div v-else class="workspace">
                <div class="toolbar glass-card">
                    <div class="tool-group">
                        <el-button @click="addBlankPage"><el-icon>
                                <Plus />
                            </el-icon> 插入空白页</el-button>
                        <el-button @click="selectAll">全选</el-button>
                        <el-button type="danger" :disabled="selectedPages.length === 0" @click="deleteSelected">
                            <el-icon>
                                <Delete />
                            </el-icon> 删除选中 ({{ selectedPages.length }})
                        </el-button>
                    </div>
                    <div class="tool-group">
...
import { ArrowLeft, Grid, Download, Plus, Delete, Document, Loading } from '@element-plus/icons-vue'
        ...
        for (const page of pages.value) {
            if (page.isBlank) {
                ...
                const [width, height] = pdfLibDoc.value ?
                    [pdfLibDoc.value.getPages()[0]?.getWidth() || 595, pdfLibDoc.value.getPages()[0]?.getHeight() || 842] :
                    [595, 842]

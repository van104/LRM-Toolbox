                        ...
                        <div class="repair-status glass-card">
                            ...
                            <div v-if="repairLog.length > 0" class="repair-log">
                                ...
                                <div class="log-content">
                                    <div v-for="(log, idx) in repairLog" :key="idx" class="log-item" :class="log.type">
                                        <el-icon>
...
v-if="status === 'success'" type="success" size="large"
                            ...
                            <el-button v-if="status === 'failed'" type="warning" size="large" @click="retryRepair">
                                重试修复
                            </el-button>
                        </div>
                    </div>
...
import { ArrowLeft, FirstAidKit, Document, Download, Check, Close, Warning, Loading } from '@element-plus/icons-vue'
...
const statusIcon = computed(() => {
    switch (status.value) {
        case 'ready': return 'FirstAidKit'
        case 'repairing': return 'Loading'
        case 'success': return 'Check'
        case 'failed': return 'Close'
        default: return 'FirstAidKit'
    }
})

const statusTitle = computed(() => {
    switch (status.value) {
        case 'ready': return '准备修复'
        case 'repairing': return '正在修复...'
        case 'success': return '修复成功'
...
const statusMessage = computed(() => {
    switch (status.value) {
        case 'ready': return '点击下方按钮开始尝试修复此 PDF 文件'
        case 'repairing': return '正在分析和修复文件结构，请稍候...'
        case 'success': return 'PDF 已成功修复！您可以下载修复后的文件。'
        case 'failed': return '无法修复此 PDF 文件，可能损坏过于严重。'
        default: return ''
    }
})

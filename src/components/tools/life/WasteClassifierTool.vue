...
v-model="searchQuery" placeholder="搜索垃圾名称，如：电池、剩菜、纸箱..." size="large" class="search-input"
                ...
                <div v-if="searchResult.length > 0" class="search-results">
                    <div v-for="item in searchResult" :key="item.name" class="result-item" @click="showDetail(item)">
            ...
            <div class="categories-grid">
                <div v-for="cat in categories" :key="cat.name" class="category-card" :style="{ '--color': cat.color }">
...
import { ref } from 'vue'
import {
    Back, Search, Opportunity, Picture, Microphone,
    Delete, Bowl, Warning, Grid

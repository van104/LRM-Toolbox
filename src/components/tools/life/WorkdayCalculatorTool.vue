...
v-model="endDate" type="date" placeholder="结束日期" class="full-width"
                ...
                <section class="holidays-panel glass-card">
                    ...
                    <div class="tabs">
                        <button :class="{ active: activeTab === 'holiday' }" @click="activeTab = 'holiday'">节假日
                            (休息)</button>
                        <button :class="{ active: activeTab === 'work' }" @click="activeTab = 'work'">补班日 (工作)</button>
                    </div>
                        ...
                        <div class="list-container">
                            <ul v-if="currentList.length" class="date-list">
                                <li v-for="d in currentList" :key="d">
                                    {{ d }}
                                    <button class="del-btn" @click="removeDate(d)"><el-icon>
                                            <Close />
                                        </el-icon></button>
                                </li>
                            </ul>
...
const weekendDays = ref(0)
const holidayDays = ref(0)
const makeupDays = ref(0)
...
const workdayCount = computed(() => {
    ...
    weekendDays.value = weekend
    holidayDays.value = holi
    makeupDays.value = makeup

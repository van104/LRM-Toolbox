            ...
            <section class="table-section glass-card">
                ...
                <div class="table-wrapper">
                    <table class="size-table">
                        ...
                        <tbody>
                            <tr v-for="(row, idx) in currentTableData" :key="idx" :class="{ active: isRowActive(row) }">
...
class="disclaimer-card"
                ...
                <span>尺码对照表为通用参考标准。由于不同品牌、款式的剪裁差异，建议结合具体商品的尺寸测量表或咨询客服进行选择。</span>
            </div>
        </div>
...
const tableKey = computed(() => {
    if (currentCategory.value === 'men' && currentSubCategory.value === 'clothes') return 'men_clothes'
    if (currentCategory.value === 'men' && currentSubCategory.value === 'suit') return 'men_clothes' 

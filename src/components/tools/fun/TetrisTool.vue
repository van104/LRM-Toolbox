<template>
  <div class="tetris-tool">
      ...
      <div class="game-wrapper-outer">
            ...
            <div class="panel-card glass-card">
              ...
              <div class="mini-grid">
                <div v-if="holdPiece" class="piece-preview">
                  <div
                    v-for="(row, rIndex) in getPieceGrid(holdPiece)"
                    :key="rIndex"
                    class="preview-row"
                  >
                    <div
            ...
            <div class="game-board">
              <div v-for="(row, rIndex) in displayGrid" :key="rIndex" class="board-row">
                <div
                  ...
                  :style="getCellStyle(cell, rIndex, cIndex)"
                ></div>
              </div>
            ...
            <div class="panel-card glass-card">
              ...
              <div class="mini-grid">
                <div v-if="nextPiece" class="piece-preview">
                  <div
                    v-for="(row, rIndex) in getPieceGrid(nextPiece)"
                    :key="rIndex"
                    class="preview-row"
                  >
                    <div
        ...
        <div class="mobile-controls">
          ...
          <div class="action-buttons">
            <button class="control-btn rotate-btn" @touchstart.prevent="rotate" @click="rotate">
              ↻
            </button>
            <button class="control-btn drop-btn" @touchstart.prevent="hardDrop" @click="hardDrop">
              ⤓
            </button>
          </div>
          ...
          >
            ...
            <button class="control-btn small-btn" @click="togglePause">
              {{ isPaused ? '▶' : '||' }}
            </button>
          </div>
        </div>
    ...
    <footer class="footer">© 2026 LRM工具箱 - 俄罗斯方块</footer>
  </div>
</template>
...
<script setup>
  ...
  function getCellStyle(cell, rIndex, cIndex) {
    if (isGhost(rIndex, cIndex)) {
      return {
        backgroundColor: activePiece.value
          ? hexToRgba(activePiece.value.color, 0.2)
          : 'transparent',
  ...
  function isGhost(r, c) {
    ...
    const { x, shape } = activePiece.value;
  ...
  function update(time) {
    if (!isGameStarted.value || isPaused.value || gameOver.value) {
      gameLoopInterval = requestAnimationFrame(update);
      return;
    }

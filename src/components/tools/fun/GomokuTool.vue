<template>
  <div class="gomoku-tool">
            ...
            <div class="board" :style="boardStyle">
              <svg class="grid-lines" :viewBox="`0 0 ${boardPixelSize} ${boardPixelSize}`">
                ...
                <line
                  ...
                  :y2="boardPixelSize - cellSize / 2"
                  stroke="#8B4513"
                  stroke-width="1"
                />

                <circle
                  v-for="(pos, idx) in starPoints"
                  :key="'star' + idx"
                  :cx="cellSize / 2 + pos.x * cellSize"
                  :cy="cellSize / 2 + pos.y * cellSize"
                  r="4"
                  fill="#8B4513"
                />
              </svg>
    ...
    <footer class="footer">© 2026 LRM工具箱 - 五子棋</footer>
  </div>
</template>
...
<script setup>
  ...
  const initBoard = () => {
    board.value = Array(boardSize.value)
      .fill(null)
      .map(() => Array(boardSize.value).fill(null));

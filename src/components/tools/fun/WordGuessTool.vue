<template>
  <div class="word-guess-tool">
      ...
      <div class="game-container">
        <div class="status-bar">
          <div class="mode-switch">
            <span :class="{ active: mode === 'english' }" @click="switchMode('english')"
              >英文 (5字母)</span
            >
            <span :class="{ active: mode === 'chinese' }" @click="switchMode('chinese')"
              >中文 (4字成语)</span
            >
          </div>
        </div>
        ...
        <div v-if="mode === 'chinese' && gameState === 'playing'" class="chinese-input-area">
          <input
            ref="chineseInputRef"
            v-model="chineseInputDisplay"
            type="text"
            placeholder="点击此处输入四字成语，按回车确认"
            class="real-input"
            maxlength="4"
            @input="handleChineseInput"
            @keydown.enter="submitGuess"
          />
          <button class="submit-btn" :disabled="currentGuess.length !== 4" @click="submitGuess">
            确认提交
          </button>
        </div>
    ...
    <footer class="footer">© 2026 LRM工具箱 - 猜词游戏</footer>
  </div>
</template>

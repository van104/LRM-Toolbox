    ...
    <main class="main-content">
          ...
          <div class="parent-select">
            ...
            <div class="blood-options">
              <button
                ...
                :key="'f-' + type"
                :class="{ active: fatherType === type }"
                @click="fatherType = type"
              >
                {{ type }}
              </button>
            </div>
          ...
          <div class="parent-select">
            ...
            <div class="blood-options">
              <button
                v-for="type in bloodTypes"
                :key="'m-' + type"
                :class="{ active: motherType === type }"
                @click="motherType = type"
              >
                {{ type }}
              </button>
            </div>
        ...
        <div v-if="result" class="result-section">
          <div class="result-box possible">
            ...
            <div class="blood-list">
              <span v-for="t in result.possible" :key="t" class="blood-badge possible"
                >{{ t }}型</span
              >
            </div>
          </div>
          ...
          <div class="result-box impossible">
            ...
            <div class="blood-list">
              <span v-for="t in result.impossible" :key="t" class="blood-badge impossible"
                >{{ t }}型</span
              >
            </div>
          </div>
      ...
      <div class="tips-card">
        ...
        <div class="tip-content">
          <p>遗传规律说明：</p>
          <p>
            血型遗传是由位于染色体上的基因控制的。人体细胞中的染色体是成对的，一个来自父亲，一个来自母亲。
          </p>
          <p>
            注：此工具仅计算 ABO 血型系统，不包含 Rh
            因子（阴性/阳性）。结果仅供科普参考，医学判定请以医院检测为准。
          </p>
        </div>
      </div>
    </main>

    <footer class="footer">© 2026 LRM工具箱 - 血型遗传计算器</footer>
  </div>
</template>

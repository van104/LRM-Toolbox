<template>
  <div class="sudoku-tool">
      ...
      <div class="game-container">
        ...
        <div class="numpad">
          <button
            ...
            :key="n"
            class="num-btn"
            :class="{ 'is-complete': assistMode && isNumberComplete(n) }"
            @click="inputNumber(n)"
          >
            {{ n }}
          </button>
        ...
        <div v-else class="history-list">
          <div v-for="(record, index) in gameHistory" :key="index" class="history-item">
            <div class="record-main">
              <span class="record-difficulty" :class="record.difficulty">{{
    ...
    <footer class="footer">© 2026 LRM工具箱 - 数独</footer>
  </div>
</template>
...
<script setup>
  ...
  const createPuzzle = (sol, emptyCount) => {
    const puzzle = sol.map(row =>
      row.map(val => ({ value: val, fixed: true, notes: [], error: false, hint: false }))
    );
    const positions = [];
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {

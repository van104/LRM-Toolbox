    ...
    <main class="main-content">
      <div v-if="gameStage === 'intro'" class="start-screen fade-in">
        <div class="card glass-panel">
          ...
          <div class="actions">
            <el-button type="primary" size="large" @click="startTalentSelection"
              >新的人生 (抽卡)</el-button
            >
          </div>
        </div>
      ...
      <div v-else-if="gameStage === 'talent'" class="start-screen fade-in">
        <div class="card glass-panel wide-panel">
          ...
          <div class="actions">
            ...
            <el-button type="primary" :disabled="!selectedTalent" @click="confirmTalentAndNext"
              >下一步</el-button
            >
          </div>
        </div>
      ...
      <div v-else-if="gameStage === 'points'" class="start-screen fade-in">
        <div class="card glass-panel">
          ...
          <div class="stats-allocation">
            <div class="alloc-row">
              ...
              <el-slider
                ...
                show-input
                class="alloc-slider"
                @input="val => handlePointChange('health', val)"
              />
            </div>
            <div class="alloc-row">
              <span class="label">
              ...
              <el-slider
                ...
                show-input
                class="alloc-slider"
                @input="val => handlePointChange('smarts', val)"
              />
            </div>
            <div class="alloc-row">
              <span class="label">
              ...
              <el-slider
                ...
                show-input
                class="alloc-slider"
                @input="val => handlePointChange('looks', val)"
              />
            </div>
            <div class="alloc-row">
              <span class="label">
              ...
              <el-slider
                ...
                show-input
                class="alloc-slider"
                @input="val => handlePointChange('happiness', val)"
              />
            </div>
          </div>
          ...
          <div class="actions">
            ...
            <el-button type="primary" :disabled="availablePoints < 0" @click="startGame"
              >开始人生</el-button
            >
          </div>
        </div>
      ...
      <div v-else-if="gameStage === 'game'" class="game-world fade-in">
        <div class="hud-panel glass-panel">
          ...
          <div class="hud-item assets">
            <div class="asset-row" title="现金">
              ...
              ￥{{ gameState.wealth.toLocaleString() }}
            </div>
        ...
        <div class="viewport">
          <div
            ...
            class="city-scene"
            :style="{ backgroundImage: cityMapUrl }"
          >
          ...
          <div v-else class="dashboard-scene">
            ...
            <div class="right-col glass-panel log-container">
              <div class="log-header">
                <h3>人生日志</h3>
                <el-tag v-if="gameState.degree !== 'none'"
                  >学历: {{ getDegreeName(gameState.degree) }}</el-tag
                >
              </div>
                ...
                <div v-for="(log, i) in currentLogs" :key="i" class="log-item fade-in-up">
                  ...
                  <div class="log-content">
                    ...
                    <div
                      v-if="
                        log.choices &&
                        log === gameState.log[gameState.log.length - 1] &&
                        pendingChoice
                      "
                      class="log-choices"
                    >
      ...
      >
        <div v-if="currentBuilding === 'bank'">
          <div class="bank-ui">
            ...
            <div class="bank-actions">
              ...
              <div class="btn-group">
                <el-button type="primary" @click="bankDeposit"
                  >存入 (年利{{ (lifeData.bank.depositRates.fixed1 * 100).toFixed(1) }}%)</el-button
                >
        ...
        <div v-if="currentBuilding === 'vehicles'">
          <div class="shop-grid">
            <div v-for="car in lifeData.vehicles" :key="car.id" class="shop-item card">
              ...
              <el-button
                type="primary"
                :disabled="gameState.wealth < car.price || hasCar(car.id)"
                @click="buyCar(car)"
              >
        ...
        <div v-if="currentBuilding === 'school'">
          <div class="school-list">
            <div
              v-for="edu in lifeData.education.levels"
              :key="edu.id"
              class="edu-item card"
            ...
            >
              ...
              <p v-else>义务教育 (免费)</p>
              <div class="reqs-list">
                <span v-if="edu.req" :class="{ 'text-danger': gameState.smarts < edu.req.smarts }"
                  >智力 > {{ edu.req.smarts }}</span
                >
              </div>
              <el-button
                v-if="!hasDegree(edu.id)"
                :type="checkEduReq(edu) ? 'primary' : 'info'"
                :disabled="
                  !checkEduReq(edu) ||
                  (edu.cost > gameState.wealth && edu.cost > 0) ||
                  gameState.hasStudied
                "
                @click="enroll(edu)"
              >
            ...
            <div class="job-list">
              ...
              >
                ...
                <div class="reqs">
                  <span
                    v-if="job.req.smarts"
                    :class="{ 'text-danger': gameState.smarts < job.req.smarts }"
                    >智力{{ job.req.smarts }}+
                  </span>
                  <span
                    v-if="job.req.looks"
                    :class="{ 'text-danger': gameState.looks < job.req.looks }"
                    >颜值{{ job.req.looks }}+
                  </span>
                  <span
                    v-if="job.req.health"
                    :class="{ 'text-danger': gameState.health < job.req.health }"
                    >健康{{ job.req.health }}+
                  </span>
                </div>
                <el-button
                  size="small"
                  :type="checkJobReq(job) ? 'primary' : 'info'"
                  :disabled="!checkJobReq(job)"
                  @click="applyJob(job)"
                >

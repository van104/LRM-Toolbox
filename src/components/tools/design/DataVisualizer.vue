...
width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
    ...
    <footer class="tool-footer">
      <p>本工具仅供数据可视化演示与学习。所有数据均在本地浏览器处理，不会上传至云端服务器。请勿上传敏感隐私数据。</p>
...
function updateChartData() {
  ...
  chartInstance.data.datasets[0].backgroundColor = config.chartType === 'line' ? colors[0] : colors;
  ...
  if (chartInstance.options.scales.x) chartInstance.options.scales.x.grid.display = config.showGrid;
  if (chartInstance.options.scales.y) chartInstance.options.scales.y.grid.display = config.showGrid;
...
function generateColors(count, themeName) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const base = baseColors[0].replace(/[^0-9,]/g, ''); 
    const rgb = base.split(',').slice(0, 3).join(',');
...
async function exportImage() {
  ...
  exportConfig.options.scales.x && (exportConfig.options.scales.x.ticks = { font: { size: 12 * scale } });
  exportConfig.options.scales.y && (exportConfig.options.scales.y.ticks = { font: { size: 12 * scale } });

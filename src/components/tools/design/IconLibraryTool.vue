            ...
            <div v-for="lib in libraries" :key="lib.name" class="lib-card glass-card">
                <div class="lib-header">
                    <div class="lib-icon-wrapper" :style="{ background: lib.colorBg }">
                        <component :is="lib.iconComponent" v-if="lib.iconComponent" :style="{ color: lib.colorText }" />
                        <span v-else class="text-icon" :style="{ color: lib.colorText }">{{ lib.name[0] }}</span>
                    </div>
...
<script setup>
import { ref } from 'vue'
import { Search, Close, Right, StarFilled, Picture, Back } from '@element-plus/icons-vue'
...
const libraries = [
    {
        name: 'Iconfont',
        description: '阿里巴巴矢量图标库，国内功能最强大、图标内容最丰富的矢量图标平台。支持 SVG、Font、PNG 等格式下载。',
    ...
    {
        name: 'Font Awesome',
        description: '全球最流行的图标库，提供海量图标，基于 CSS 和 Less。拥有庞大的社区支持和丰富的应用场景。',
    ...
    {
        name: 'Feather Icons',
        description: '简洁、精美的开源图标集。强调简洁、一致性和可读性，非常适合现代简洁风格的 UI 设计。',
    ...
    },
    {
        ...
        description: 'Google 推出的最新图标系统，支持可变字体（Variable Font），可调整粗细、填充、字重等属性。',

          ...
          <div class="preview-grid">
            <div v-for="(file, index) in processedFiles" :key="index" class="preview-item">
              ...
              <div class="preview-info">
                <div class="file-meta">
                  <span class="original-name" :title="file.originalName">{{ file.originalName }}</span>
      ...
      <section class="intro-section">
        ...
        <div class="features-list">
          ...
          <div class="feature-item">
            ...
            <div>
              <h4>格式生成</h4>
              <p>不仅仅是重命名，更改后缀名可方便识别（注意：实际格式转换需要转换工具，本工具主要用于文件整理）。</p>
            </div>
          </div>
          <div class="feature-item">
            <div class="icon">🔒</div>
...
const processedFiles = computed(() => {
  return files.value.map((item, index) => {
    ...
    return {
      ...item,
      generatedBase: generateBase(item.file, index, true), 
...
function generateBase(file, index, forceAuto = false) {
  let baseName = config.prefix.trim();
  const originalNameWithExt = file.name;
  const originalName = originalNameWithExt.substring(0, originalNameWithExt.lastIndexOf('.')) || originalNameWithExt;
...
function generateParts(item, index) {
  let base = generateBase(item.file, index);

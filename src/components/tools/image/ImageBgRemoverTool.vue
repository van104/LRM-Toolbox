...
type="primary" size="large" :disabled="!isConfigValid"
                            ...
                            <p v-if="!isConfigValid" class="config-warn">请先在右侧配置 API 服务商信息，或使用手动模式</p>
                        </div>
                ...
                <div class="settings-panel glass-card">
                    <div class="panel-section">
                        ...
                        <div v-if="apiConfig.provider === 'builtin'" class="provider-info">
                            ...
                            <p class="text-xs mt-1 color-blue">注：内置模式默认输出预览画质 (0.25MP)。如需高清，请配置自己的密钥并开启“高清”选项。</p>
                            <p class="text-xs mt-1 color-blue">无需配置，直接点击“开始 AI 抠图”即可。</p>
                        </div>

                        <div v-if="apiConfig.provider === 'baidu'" class="provider-info">
                            <p>1. 前往 <a href="https://console.bce.baidu.com/ai/" target="_blank">百度智能云控制台</a>
                                创建“人像分割”或“通用物体分割”应用。</p>
                            <p>2. 获取 API Key 和 Secret Key 填入下方。</p>
                        </div>

                        <div v-if="apiConfig.provider === 'remove.bg'" class="provider-info">
                            <p>1. 前往 <a href="https://www.remove.bg/api" target="_blank">Remove.bg官网</a> 注册并获取 API Key
                            </p>
...
v-model="apiConfig.customHeaders" type="textarea" :rows="3"
                    ...
                    <div class="panel-section">
                        ...
                        <div class="settings-group">
                            ...
                            <el-input v-model="outputConfig.filename" placeholder="输入自定义文件名 (不含后缀)" clearable>
...
<script setup>
import { ref, reactive, computed, onUnmounted, nextTick, watch, onMounted } from 'vue'
import { useRouter } from 'vue-router'
import { ElMessage, ElMessageBox } from 'element-plus'
import {
    ArrowLeft, Download, PictureFilled,
    MagicStick, Loading, InfoFilled, Lock,
    DArrowLeft, DArrowRight, Edit, RefreshLeft
...
const handleUpload = (e) => {
    ...
    outputConfig.filename = lastDotIndex !== -1 ? file.name.substring(0, lastDotIndex) + '_no-bg' : file.name + '_no-bg'
}
...
const callBuiltinApi = async () => {
    ...
    const response = await fetch('https://api.remove.bg/v1.0/removebg', {
        method: 'POST', headers: { 'X-Api-Key': BUILTIN_KEY }, body: formData
    })
    if (!response.ok) {
...
const updateStrokeImage = debounce(async () => {
    ...
    try {
        ...
        if (strokeUrl.value && strokeUrl.value !== resultUrl.value) URL.revokeObjectURL(strokeUrl.value)
...
watch([resultUrl, () => strokeConfig.enabled, () => strokeConfig.color, () => strokeConfig.width], () => {
    updateStrokeImage()
})

        ...
        <div class="tool-content">
            <aside class="sidebar">
                <div class="nav-menu">
                    ...
                    <button :class="['nav-item', { active: currentTab === 'insert' }]" @click="currentTab = 'insert'">
                        <el-icon>
                            <DocumentAdd />
                        </el-icon> CSV转 INSERT
                    </button>
                    <button :class="['nav-item', { active: currentTab === 'create' }]" @click="currentTab = 'create'">
                        <el-icon>
                            <Coin />
                        </el-icon> JSON转 CREATE
                    </button>
                    <button :class="['nav-item', { active: currentTab === 'smart' }]" @click="currentTab = 'smart'">
                        <el-icon>
                            <MagicStick />
                        </el-icon> SQL 智能填充
                    </button>
            ...
            <main class="main-panel">
                ...
                <div v-if="currentTab === 'in'" class="tool-pane">
                    ...
                    <div class="split-view">
                        <textarea v-model="inInput" class="code-editor" placeholder="输入列表，每行一个..."></textarea>
                ...
                <div v-if="currentTab === 'insert'" class="tool-pane">
                    ...
                    <div class="split-view">
                        ...
                        <div class="result-box">
                            <textarea v-model="insertOutput" class="code-editor result-editor" readonly></textarea>
                        </div>
                    </div>
                    <div class="actions-row">
                ...
                <div v-if="currentTab === 'create'" class="tool-pane">
                    ...
                    <div class="split-view">
                        ...
                        <div class="result-box">
                            <textarea v-model="createOutput" class="code-editor result-editor" readonly></textarea>
                        </div>
                    </div>
                    <div class="actions-row">
...
import { ArrowLeft, List, DocumentAdd, Coin, Right, UploadFilled, MagicStick } from '@element-plus/icons-vue'
...
function genIN() {
    ...
    const lines = inInput.value.split('\n').map(l => l.trim()).filter(l => l)
...
function genINSERT() {
    ...
    if (insertOpts.batch) {
        ...
        insertOutput.value = valuesList.map(v => `INSERT INTO ${table} (${colsStr}) VALUES ${v};`).join('\n')
    }
}
...
function genSmartData() {
    ...
    selectedTables.value.forEach(tName => {
        ...
        const keys = table.columns.map(c => c.name).map(k => `\`${k}\``).join(', ')
...
function mapTypeToMock(type) {
    ...
    if (type.includes('DECIMAL') || type.includes('FLOAT') || type.includes('DOUBLE')) return '@float(0, 1000, 2, 2)'

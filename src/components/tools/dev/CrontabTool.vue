...
const CronInput = defineComponent({
  props: ['modelValue', 'unit', 'max', 'min', 'labels'],
  ...
  setup(props, { emit }) {
    ...
    const updateValue = () => {
      ...
      else if (type.value === 'list') val = list.value.length ? list.value.sort((a, b) => a - b).join(',') : '*'
      emit('update:modelValue', val)
    }

    watch([type, range, step, list], updateValue, { deep: true })

    return () => h('div', { class: 'cron-input-inner' }, [
      h('el-radio-group', {
      ...
      type.value === 'range' ? h('div', { class: 'config-row' }, [
        h('el-input-number', { modelValue: range.value[0], 'onUpdate:modelValue': v => range.value[0] = v, min: props.min || 0, max: props.max, size: 'small' }),
        h('span', { class: 'mx-2' }, '至'),
        h('el-input-number', { modelValue: range.value[1], 'onUpdate:modelValue': v => range.value[1] = v, min: props.min || 0, max: props.max, size: 'small' }),
        h('span', { class: 'ml-2' }, props.unit)
      ]) : null,

      type.value === 'step' ? h('div', { class: 'config-row' }, [
        h('span', '从'),
        h('el-input-number', { modelValue: step.value.start, 'onUpdate:modelValue': v => step.value.start = v, min: props.min || 0, max: props.max, size: 'small', class: 'mx-2' }),
        h('span', '开始，每'),
        h('el-input-number', { modelValue: step.value.interval, 'onUpdate:modelValue': v => step.value.interval = v, min: 1, max: props.max, size: 'small', class: 'mx-2' }),
        h('span', props.unit + '执行一次')
      ]) : null,

      type.value === 'list' ? h('el-checkbox-group', {
        ...
        return h('el-checkbox', { label: val, border: true, size: 'small' }, props.labels ? props.labels[val] : val)
      })) : null
    ])
  }
})
...
const humanDescription = computed(() => {
  ...
  if (week.value !== '?' && week.value !== '*') {
    const ws = week.value.split(',').map(v => weekLabels[v]).join('、')
    desc += `(限${ws}) `
  }

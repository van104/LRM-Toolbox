    ...
    <div class="api-tester-tool">
            ...
            <div class="debugger-layout">
                    ...
                    <el-tabs v-model="activeRequestTab">
                        ...
                        <el-tab-pane label="请求体 (Body)" name="body" :disabled="method === 'GET'">
                            <div class="body-config">
                                ...
                                <div v-if="bodyType === 'none'" class="empty-body">
                                    该请求不含 Body 内容
                                </div>
                            </div>
                        </el-tab-pane>
                ...
                <section class="response-panel glass-card">
                    <div class="response-header" v-if="response">
                        ...
                        <div class="time-wrap">耗时: <b>{{ response.time }} ms</b></div>
                        <div class="size-wrap">大小: <b>{{ response.size }}</b></div>
                    </div>
        ...
        <el-dialog v-model="corsDialogVisible" title="关于跨域(CORS)请求的说明" width="500px">
            <div class="cors-tip-content">
                <p>由于这是一个纯前端工具，受浏览器安全策略限制，发送请求时可能会触发<b>跨域错误 (CORS Error)</b>：</p>
                <ul>
                    <li>如果目标服务器未配置 <code>Access-Control-Allow-Origin</code> 头部，请求将被浏览器拦截。</li>
                    <li>非 HTTPS 站点请求 HTTPS 接口可能会被拦截。</li>
                    <li>建议测试支持 CORS 的公开接口，或用于调试本地允许跨域的服务。</li>
                </ul>
                <p class="mt-4"><b>解决方法：</b></p>
                <p>1. 服务端设置允许跨域；<br>2. 调试时使用浏览器插件（如 Allow CORS）临时禁用安全限制。</p>
            </div>
        </el-dialog>

        <footer class="footer">
            © 2026 LRM工具箱 - API 接口调试器
        </footer>
    </div>
</template>
...
import { Back, Plus, Delete, Warning, ArrowDown } from '@element-plus/icons-vue'
...
const sendRequest = async () => {
    ...
    try {
        ...
        response.value = {
            ...
            size: JSON.stringify(data).length > 1024 ? (JSON.stringify(data).length / 1024).toFixed(2) + ' KB' : JSON.stringify(data).length + ' B',
            data: data
        }

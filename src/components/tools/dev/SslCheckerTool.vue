            ...
            <div class="glass-card detail-card mb-4">
              ...
              <el-descriptions :column="1" border>
                ...
                <el-descriptions-item label="备用名称 (SANs)">
                  <div class="sans-tags">
                    <el-tag v-for="tag in sans" :key="tag" size="small" type="info">{{ tag }}</el-tag>
                    <span v-if="!sans.length" class="text-secondary text-xs">通过 PEM 解析可能受限</span>
                  </div>
                </el-descriptions-item>
              </el-descriptions>
            </div>
  ...
  try {
    ...
    const findAllCNs = (data) => {
      ...
      while ((pos = data.indexOf(cnOID, pos)) !== -1) {
        ...
        if ([0x0c, 0x13, 0x16, 0x14].includes(tag)) {
          ...
          const str = data.substring(strPos + 1 + info.offset, strPos + 1 + info.offset + info.len)
    ...
    if (dateMatches && dateMatches.length >= 2) {
      const formatDate = (s) => {
        ...
        if (isGeneralized) {
          y = s.substring(0, 4)
          m = s.substring(4, 6); d = s.substring(6, 8)
          hh = s.substring(8, 10); mm = s.substring(10, 12); ss = s.substring(12, 14)
        } else {
          const yy = parseInt(s.substring(0, 2))
          y = (yy < 50 ? '20' : '19') + s.substring(0, 2)
          m = s.substring(2, 4); d = s.substring(4, 6)
          hh = s.substring(6, 8); mm = s.substring(8, 10); ss = s.substring(10, 12)
        }
    ...
    if (binary.charCodeAt(p) === 0x30) {
      ...
      if (binary.charCodeAt(p) === 0x30) {
        ...
        if (binary.charCodeAt(currentPos) === 0x02) {
          ...
          sn = toHex(binary.substring(currentPos + 1 + snInfo.offset, currentPos + 1 + snInfo.offset + snInfo.len))
            .toUpperCase()
            .match(/.{2}/g)
            ?.join(':') || 'N/A'
        }

                ...
                <div class="info-card glass-card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="info-row">
                            ...
                            <span class="value" :class="{ online: browserInfo.online }">{{ browserInfo.online ? '在线' :
                                '离线' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="info-card glass-card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="info-row">
                            ...
                            <span class="value">{{ connectionInfo.downlink ? connectionInfo.downlink + ' Mbps' : '未知'
                                }}</span>
                        </div>
                        <div class="info-row">
                            <span class="label">往返延迟</span>
                            <span class="value">{{ connectionInfo.rtt ? connectionInfo.rtt + ' ms' : '未知' }}</span>
                        </div>
                        <div class="info-row">
...
import { ArrowLeft, Refresh, Connection, Monitor, ChromeFilled, Promotion, Clock, Location, Opportunity } from '@element-plus/icons-vue'
...
const screenInfo = reactive({
    width: 0, height: 0, availWidth: 0, availHeight: 0, pixelRatio: 1, colorDepth: 24
})

const browserInfo = reactive({
    name: '', version: '', language: '', platform: '', cookieEnabled: false, online: true
})

const connectionInfo = reactive({
    type: '', effectiveType: '', downlink: 0, rtt: 0, saveData: false
})

const timeInfo = reactive({
    localTime: '', timezone: '', utcOffset: ''
})

const batteryInfo = reactive({
    supported: false, level: 100, charging: false
})
...
const getBrowserInfo = () => {
    ...
    if (/Edg\/(\d+)/.test(ua)) { browserInfo.name = 'Edge'; browserInfo.version = RegExp.$1 }
    else if (/Chrome\/(\d+)/.test(ua)) { browserInfo.name = 'Chrome'; browserInfo.version = RegExp.$1 }
    else if (/Firefox\/(\d+)/.test(ua)) { browserInfo.name = 'Firefox'; browserInfo.version = RegExp.$1 }
    else if (/Safari\/(\d+)/.test(ua) && !/Chrome/.test(ua)) { browserInfo.name = 'Safari'; browserInfo.version = RegExp.$1 }
}
...
const getBatteryInfo = async () => {
    if ('getBattery' in navigator) {
        ...
        } catch { batteryInfo.supported = false }
    }
}

<template>
  <div class="git-cheatsheet">
    ...
    <footer class="footer">© 2026 LRM工具箱 - Git 常用命令备忘录</footer>
  </div>
</template>
      ...
      items: [
        {
          ...
          code: 'git reset <file>',
          tips: '相当于 git restore --staged <file>'
        },
        {
          desc: '回退到指定版本 (保留修改)',
        ...
        },
        {
          ...
          tips: '慎用！工作区和暂存区都会被重置'
        },
        {
          desc: '撤销指定文件的修改',
          code: 'git checkout -- <file>',
          tips: '新版本建议用 git restore <file>'
        },
    ...
    return gitData
      .map(cat => ({
        ...
        items: cat.items.filter(
          i => i.desc.toLowerCase().includes(q) || i.code.toLowerCase().includes(q)
        )
      }))

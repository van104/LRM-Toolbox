...
:class="['tab-btn', { active: currentTab === 'obfuscate' }]"
                    ...
                    <div class="tools-content">
                        ...
                        <div v-if="currentTab === 'mask'" class="tool-group">
                            ...
                            <div class="control-row">
                                <label>替换字符:</label>
                                <input v-model="maskOptions.char" class="text-input" placeholder="*" maxlength="1" />
                            </div>
                            <button class="action-btn primary" @click="doMask('custom')">执行自定义自定义脱敏</button>
                        </div>
                ...
                <div v-if="showHelp" class="modal-overlay" @click="showHelp = false">
                    <div class="modal-content" @click.stop>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="help-section">
                                <h4>👁️ 混淆模式</h4>
                                <p>• <b>防复制</b>：插入不可见的零宽字符，复制后粘贴到搜索框可能无法搜索到原词，用于防爬虫或防匹配。</p>
                            </div>
                        </div>
                    </div>
                </div>
...
function doMorse(mode) {
    if (!inputText.value) return showToast('请先输入文本')
    if (mode === 'enc') {
        outputText.value = inputText.value.toUpperCase().split('').map(c => MORSE_CODE[c] || c).join(' ')
    } else {
